...
def generate_certificate_data_string(sertifikat_obj):
    ...
    # Perbaikan untuk tanggal_terbit_str
    tanggal_terbit_str = "N/A" # Default jika tanggal_terbit None atau bukan date
    if isinstance(sertifikat_obj.tanggal_terbit, date): # Cukup periksa instance dari datetime.date
        tanggal_terbit_str = sertifikat_obj.tanggal_terbit.strftime('%Y-%m-%d')
    elif sertifikat_obj.tanggal_terbit is not None: # Jika bukan date tapi ada (misal string, walau seharusnya tidak)
        tanggal_terbit_str = str(sertifikat_obj.tanggal_terbit)


    return (f"{sertifikat_obj.nomor_sertifikat}|"
            f"{nama_pemilik}|"
            f"{sertifikat_obj.nama_kompetensi}|"
            f"{tanggal_terbit_str}|" # Gunakan hasil yang sudah diformat
